...
## V2!***
# V2 of this file updates functions to use the new Rforecastio package. 
# the apikep must now be stores in an .Renviron file in your home director
# create the file or edit it such that one line contains 
# FORECASTIO_API_KEY = [our API key]
...
FIOWeatherGrabLoad <- function(latitude, longitude, dates, locId = NA, dbcon, apikey, verbose = F){
  ...
  for (d in 1 : length(dates)){    
    ...
    fio.list <- get_forecast_for(latitude = latitude, longitude = longitude,
                              timestamp = format(dates[d], '%Y-%m-%dT%H:%M:%S%z') )
    
    fio.list$daily.df  <- as.data.frame(fio.list$daily)
    fio.list$hourly.df <- as.data.frame(fio.list$hourly)
    ...
    dayId <- as.numeric(dbGetQuery(con, 'SELECT max("dayId") from weather_forecastio."dailyData";')+1)
    ...
    lfields             <- dbListFields(con,c('weather_forecastio','dailyData'))
...
DBWeatherGetHours <- function(locId, timebounds, dbcon){
  ...
  hquery     <- paste( 'SELECT *, extract(epoch from "dateTime" at time zone \'utc\')',
                       ' FROM weather_forecastio."hourlyData" WHERE "dayId" in (', dayidstr  ,');' )
